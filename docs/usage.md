...
PR 열기
-> 필요한 시점에 PR 댓글 작성
-> @오픈코드 리뷰해줘, @gpt높음 리뷰해줘, @gpt매우높음 리뷰해줘, @gpt확장 리뷰해줘, @클로드 리뷰해줘, @클로드-p 리뷰해줘, @코덱스 리뷰해줘, 또는 @최종리뷰
...
`@gpt높음`, `@gpt매우높음`, `@gpt확장`은 운영 서버의 headed Chromium +
`agbrowse`로 ChatGPT 웹 UI를 열고 GitHub 연동으로 직접 PR 댓글을 남기는
경로입니다. 현재 기본 매핑은 다음과 같습니다.

- `@gpt높음` → ChatGPT `thinking` + `extended`
- `@gpt매우높음` → ChatGPT `thinking` + `heavy`
- `@gpt확장` → ChatGPT `pro` + `extended`
...
- marker 포함 댓글/리뷰가 실제 PR에 게시되어야 성공입니다.
