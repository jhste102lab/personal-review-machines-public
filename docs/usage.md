...
- `@지피티확장` → 단일 CDP 세션, ChatGPT 추론 수준 `Pro 확장`
...
- 요청은 SQLite job queue에 저장한 뒤 기본적으로 worker 1개가 순차 처리합니다.
  ChatGPT job은 같은 브라우저 페이지를 재사용하되 매 job마다 ChatGPT `/` 경로로 이동해 새 채팅에서 시작합니다.
...
- reviewer 실행 뒤 marker가 게시되지 않으면 최대 4회까지 지수 backoff로 자동 재시도하고,
  모두 실패한 경우에만 failed 상태를 DB/log에 기록합니다.
