...
- 요청은 이전 reviewer 완료를 기다리지 않고 독립적으로 실행합니다. 외부 reviewer
  ...
  prompt 전송은 CDP 슬롯 전체에서 하나씩만 진행합니다. checkout과 prompt 준비는 병렬로
  수행하고 browser의 새 채팅 열기·전송·세션 확인만 send lane에서 대기합니다. 세션 생성/전송
  확인 후 즉시 rocket을 남기고, 다음 전송은 기본 45~75초의 랜덤 간격 뒤에 시작합니다. 이전
  generation 답변은 기다리지 않습니다. prompt 전송 전 403·네트워크 실패의 재시도 cooldown 기본값은
  90초, 150초, 300초입니다.
